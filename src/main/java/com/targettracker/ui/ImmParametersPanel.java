...
/** Embedded editor for IMM models, process noise, association, and track breaks. */
final class ImmParametersPanel extends JPanel {
    ...
    ImmParametersPanel(ImmSettings settings, Runnable onParametersChanged) {
        ...
        cvNoiseField = addModelRow(form, cvButton,
                "DCWNA acceleration PSD q (m²/s³)", defaults.cvProcessNoise());
        caNoiseField = addModelRow(form, caButton,
                "Continuous white-jerk PSD q (m²/s⁵)", defaults.caProcessNoise());
        form.add(Box.createVerticalStrut(8));
        form.add(sectionTitle("Association"));
        associationField = addField(form, "Mahalanobis distance threshold",
                defaults.associationMahalanobisThreshold());
        form.add(sectionTitle("Track break"));
        timeoutField = addField(form, "No-detection timeout (seconds)",
                defaults.timeoutSeconds());
        uncertaintyField = addField(form, "Maximum 3D covariance radius (meters)",
        ...
        rebuildTransitionMatrix(defaults.transitionProbabilityMatrix(), inputListener);
    ...
    private static JTextField addModelRow(
            JPanel panel, JToggleButton button, String processNoiseLabel, double value) {
        ...
        field.setPreferredSize(new Dimension(75, 28));
